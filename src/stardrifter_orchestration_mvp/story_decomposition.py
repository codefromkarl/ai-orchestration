...
from .protocols import IntakeAdapter
from .repository import ControlPlaneRepository


DECOMPOSITION_RESULT_MARKER = "STARDRIFTER_DECOMPOSITION_RESULT_JSON="
    ...
    *,
    ...
    if task_count <= 0:
        ...
        if fallback_generated:
            refresher(connection=connection, repo=repo)
    ...
    *,
    ...
    if not command_template:
        return DecompositionExecutionResult(
            success=False,
            outcome="blocked",
            summary="STARDRIFTER_DECOMPOSER_COMMAND is required",
    ...
    env.update(
        {
            "STARDRIFTER_STORY_ISSUE_NUMBER": str(story_issue_number),
            "STARDRIFTER_STORY_TITLE": str(story.get("story_title") or ""),
            "STARDRIFTER_STORY_REPO": repo,
            "STARDRIFTER_PROJECT_DIR": str(project_dir),
        }
    )
...
def _fallback_enabled() -> bool:
    raw = os.environ.get("STARDRIFTER_ENABLE_DEFAULT_DECOMPOSITION_FALLBACK", "")
