...
def main() -> int:
    story_issue_number = os.environ.get("STARDRIFTER_STORY_ISSUE_NUMBER", "").strip()
    repo = (
        os.environ.get("STARDRIFTER_STORY_REPO", "").strip()
    ...
    if not story_issue_number:
        raise SystemExit("STARDRIFTER_STORY_ISSUE_NUMBER is required")
    if not dsn:
