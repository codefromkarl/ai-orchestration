...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="stardrifter-orchestration-loop",
    ...
    executor_command = (
        "export STARDRIFTER_OPENCODE_TIMEOUT_SECONDS='{timeout_seconds}'; "
        "python3 -m stardrifter_orchestration_mvp.opencode_task_executor"
    ).format(
        timeout_seconds=args.opencode_timeout_seconds,
    )
    verifier_command = "python3 -m stardrifter_orchestration_mvp.task_verifier"
    ...
    while True:
        ...
        for epic_issue_number in decomposition_epic_issue_numbers:
            command = (
                "export STARDRIFTER_ORCHESTRATION_DSN='{dsn}'; "
