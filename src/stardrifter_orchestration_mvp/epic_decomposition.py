    ...
    *,
    ...
    if not command_template:
        return DecompositionExecutionResult(
            success=False,
            outcome="blocked",
            summary="STARDRIFTER_EPIC_DECOMPOSER_COMMAND is required",
    ...
    env = {
        **os.environ,
        "STARDRIFTER_EPIC_ISSUE_NUMBER": str(epic_issue_number),
        "STARDRIFTER_EPIC_REPO": repo,
...
def _load_timeout_seconds() -> int:
    raw_value = os.environ.get("STARDRIFTER_OPENCODE_TIMEOUT_SECONDS", "600").strip()
    try:
        value = int(raw_value)
    except ValueError:
        return 600
