...
def main() -> int:
    epic_issue_number = os.environ.get("STARDRIFTER_EPIC_ISSUE_NUMBER", "").strip()
    repo = (
        os.environ.get("STARDRIFTER_EPIC_REPO", "").strip()
    ...
    if not epic_issue_number:
        raise SystemExit("STARDRIFTER_EPIC_ISSUE_NUMBER is required")
    if not dsn:
