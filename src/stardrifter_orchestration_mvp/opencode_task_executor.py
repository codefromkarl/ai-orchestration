...
def main() -> int:
    ...
    resume_context = os.environ.get("STARDRIFTER_RESUME_CONTEXT", "").strip()
    if not work_id:
...
def _build_opencode_run_command(*, focus_dir: Path, prompt: str) -> list[str]:
    ...
    configured_model = os.environ.get("STARDRIFTER_OPENCODE_MODEL", "").strip()
    if configured_model:
        command.extend(["--model", configured_model])
    configured_variant = os.environ.get("STARDRIFTER_OPENCODE_VARIANT", "").strip()
...
def _load_timeout_seconds(*, bounded_mode: bool = False) -> int:
    raw = os.environ.get("STARDRIFTER_OPENCODE_TIMEOUT_SECONDS", "").strip()
    if not raw:
    ...
    try:
        ...
        raise SystemExit(
            "STARDRIFTER_OPENCODE_TIMEOUT_SECONDS must be an integer"
        ) from exc
    if value <= 0:
        raise SystemExit("STARDRIFTER_OPENCODE_TIMEOUT_SECONDS must be positive")
    return value


def _load_hard_cap_seconds(*, timeout_seconds: int, bounded_mode: bool) -> int:
    raw = os.environ.get("STARDRIFTER_OPENCODE_HARD_CAP_SECONDS", "").strip()
    if not raw:
    ...
    try:
        ...
        raise SystemExit(
            "STARDRIFTER_OPENCODE_HARD_CAP_SECONDS must be an integer"
        ) from exc
    if value <= 0:
        raise SystemExit("STARDRIFTER_OPENCODE_HARD_CAP_SECONDS must be positive")
    if value < timeout_seconds:
        raise SystemExit(
            "STARDRIFTER_OPENCODE_HARD_CAP_SECONDS must be >= STARDRIFTER_OPENCODE_TIMEOUT_SECONDS"
        )
    return value
