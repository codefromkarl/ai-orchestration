...
TRANSIENT_QUEUE_VIOLATION_CODES = {
    "path-conflict",
}
...
def _materialize_blocked_items(
    ...
    for work_id, violations in evaluation.blocked_by_id.items():
        persistent_violations = [
            ...
            if violation.code not in TRANSIENT_QUEUE_VIOLATION_CODES
        ]
        if not persistent_violations:
            continue
        repository.mark_blocked(work_id, persistent_violations)
