...
class TaskContextEngine:
    ...
    def build_context(
        ...
        *,
        ...
        related_files = self._collect_related_files(
            work_item, execution_context, workspace_path
        )
        conventions = self._load_conventions(workspace_path)
        memory_section = self._load_memory_section(work_item)
        ...
        sections: list[tuple[str, str]] = [
            ...
            (
                ...
                memory_section or "No historical memory available for this task type.",
            ),
        ]
        return self._trim_sections(sections)

    def _load_memory_section(self, work_item: WorkItem) -> str:
        ...
        try:
            with psycopg.connect(dsn, row_factory=dict_row) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        SELECT dl.failure_reason, dl.attempt_count, dl.moved_at,
                               er.summary AS last_summary
                        FROM dead_letter_queue dl
                        LEFT JOIN execution_run er ON er.id = dl.last_run_id
                        WHERE dl.resolution IS NOT NULL
                          AND dl.work_id IN (
                              SELECT wd2.depends_on_work_id
                              FROM work_dependency wd2
                              WHERE wd2.work_id = %s
                          )
                    ...
                    for row in rows:
                        ...
                        parts.append(
                            f"- Dependency failed previously: reason={reason}, attempts={attempts}"
                        )
        except Exception:
            pass
    ...
    def _load_issue_material(self, work_item: WorkItem) -> tuple[str, str]:
        ...
        return issue_excerpt, "\n".join(
            part for part in acceptance_parts if part
        ).strip()
...
class IntelligentExecutor:
    ...
    def _run_loop(
        ...
        *,
        ...
        for _ in range(self.config.max_turns):
            ...
            payload.setdefault(
                "summary", _compact_journal(journal) or "LLM execution completed"
            )
            payload.setdefault(
                "decision_required", payload.get("outcome") == "needs_decision"
            )
    ...
    def _compact_messages(
        self, *, messages: list[dict[str, Any]], journal: list[str]
    ) -> None:
        ...
        summary_message = {
            "role": "user",
            "content": (
                f"Execution history summary (compressed):\n{_compact_journal(journal)}"
            ),
        }
    ...
    def _build_failed_result(
        ...
        *,
        ...
        payload = {
            ...
            "summary": summary,
            "decision_required": reason_code
            in {"credential_required", "permission_required"},
...
class LLMVerifier:
    ...
    def __call__(
        ...
        try:
            ...
            llm_payload = _extract_terminal_payload(llm_text) or _coerce_json_object(
                llm_text
            )
...
def llm_executor_enabled(*, command_template: str) -> bool:
    return _is_truthy(
        os.environ.get("TASKPLANE_ENABLE_LLM_EXECUTOR")
    ) or command_template.strip().lower().startswith("llm://")


def llm_verifier_enabled(*, command_template: str) -> bool:
    return _is_truthy(
        os.environ.get("TASKPLANE_ENABLE_LLM_VERIFIER")
    ) or command_template.strip().lower().startswith("llm://")
...
def _run_verifier_command_if_enabled(
    *,
    ...
    if execution_context is not None:
        env["TASKPLANE_EXECUTION_CONTEXT_JSON"] = _json_dump(
            {
...
def _validate_bash_command(command: str) -> str | None:
    if _is_truthy(os.environ.get("TASKPLANE_LLM_BASH_ALLOW_UNSAFE")):
        return None
...
def _load_bash_deny_patterns() -> list[str]:
    from_env = (os.environ.get("TASKPLANE_LLM_BASH_DENY_PATTERNS") or "").strip()
    if from_env:
...
def _load_bash_allow_patterns() -> list[str]:
    from_env = (os.environ.get("TASKPLANE_LLM_BASH_ALLOW_PATTERNS") or "").strip()
    if not from_env:
        return []
    return [part.strip() for part in from_env.split(",") if part.strip()]
