...
class AgentHub:
    ...
    def __init__(self, workdir: str | None = None):
        ...
        self._agents: dict[str, RuntimeAgentSpec] = {}
        self._running_instances: dict[str, AgentInstance] = {}
        self._lock = threading.Lock()

    def register_agent(self, config: RuntimeAgentSpec) -> None:
        """
        注册 Agent 配置
    ...
    def get_agent(self, agent_name: str) -> RuntimeAgentSpec | None:
        """获取 Agent 配置"""
        return self._agents.get(agent_name)

    def list_agents(self) -> list[RuntimeAgentSpec]:
        """列出所有已注册的 Agent"""
        return list(self._agents.values())

    def get_available_agents(self) -> list[RuntimeAgentSpec]:
        """获取可用的 Agent（未在运行中）"""
        with self._lock:
            running_agent_names = {
    ...
    def execute_parallel(
        self,
        agent_configs: list[tuple[RuntimeAgentSpec, WorkItem]],
        heartbeat: Callable[[], None] | None = None,
...
def create_default_agent_hub(workdir: str | None = None) -> AgentHub:
    ...
    # Claude Code
    hub.register_agent(RuntimeAgentSpec(
...
# Backward compatibility alias. New code should use RuntimeAgentSpec.
AgentConfig = RuntimeAgentSpec
