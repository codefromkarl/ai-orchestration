...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dsn",
        default=os.environ.get("TASKPLANE_DSN", ""),
        help="PostgreSQL DSN (or set TASKPLANE_DSN)",
    )
    parser.add_argument(
        "--project-dir",
        default=os.environ.get("TASKPLANE_PROJECT_DIR", "."),
...
def _make_opencode_executor(
    *,
    ...
    class OpencodeSessionExecutor:
        def run_turn(self, request: SessionTurnRequest) -> ExecutorResult:
            env = os.environ.copy()
            env["TASKPLANE_WORK_ID"] = work_id
            env["TASKPLANE_DSN"] = dsn
            env["TASKPLANE_PROJECT_DIR"] = str(project_dir)
            if request.resume_context:
                env["TASKPLANE_RESUME_CONTEXT"] = request.resume_context

            cmd = [
                sys.executable,
                "-m",
                "taskplane.opencode_task_executor",
            ]
    ...
    return OpencodeSessionExecutor()
...
def main() -> int:
    ...
    if not args.dsn:
        print("Error: --dsn or TASKPLANE_DSN required", file=sys.stderr)
        return 1
    ...
    result = run_session_to_completion(
        ...
        resume_context_builder=build_store_backed_resume_context_builder(
            dsn=args.dsn,
            session_manager=session_manager,
        ),
