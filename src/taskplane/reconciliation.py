...
def build_reconciliation_report(
    *,
    ...
    for row in task_rows:
        ...
        elif db_status in {"blocked", "awaiting_approval"} or bool(
            row.get("db_decision_required") or False
        ):
            expected_label = "status:blocked"
        elif db_status in {"in_progress", "verifying"}:
            expected_label = "status:in-progress"
        else:
            expected_label = "status:pending"
        if github_state and (
            github_state != expected_state or github_status_label != expected_label
        ):
            task_drift.append(
                {
    ...
    for row in epic_rows:
        ...
        if db_epic_complete:
            expected_label = "status:done"
        elif execution_status in {"blocked", "needs_story_refinement"}:
            expected_label = "status:blocked"
        else:
            expected_label = "status:pending"
        if github_state and (
    ...
    *,
    ...
    for drift in report.get("epic_drift", []):
        ...
        epic_repair(
            repo=repo,
            issue_number=issue_number,
            status=_github_writeback_status_for_program_row(
                complete=bool(drift.get("db_epic_complete") or False),
                execution_status=str(drift.get("execution_status") or ""),
            ),
    ...
    for drift in report.get("story_drift", []):
        ...
        story_repair(
            ...
            issue_number=issue_number,
            status=_github_writeback_status_for_program_row(
                complete=bool(drift.get("db_story_complete") or False),
                execution_status=str(drift.get("execution_status") or ""),
            ),
            decision_required=False,
        )
...
def _github_writeback_status_for_program_row(
    *, complete: bool, execution_status: str
) -> str:
    if complete:
        return "done"
    if execution_status in {"blocked", "needs_story_refinement"}:
        return "blocked"
    if execution_status in {"in_progress", "verifying"}:
        return "in_progress"
    return "pending"
