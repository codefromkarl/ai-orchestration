...
from .artifact_store import ArtifactRecord, summarize_artifacts_for_prompt
from .models import AIConversationSummary, AIConversationTurn


def _build_structured_summary(rows: list[dict[str, Any]]) -> str:
    decisions = []
    changes = []
    issues = []
    for row in rows:
        ...
        if role == "assistant":
            if any(
                kw in content.lower()
                for kw in ("decided", "chose", "selected", "fixed")
            ):
                decisions.append(content[:200])
            elif any(
                kw in content.lower() for kw in ("error", "fail", "issue", "problem")
            ):
                issues.append(content[:200])
            else:
    ...
    if changes:
        parts.append("Changes: " + "; ".join(changes[:3]))
    if issues:
        parts.append("Issues: " + "; ".join(issues[:3]))

    return "\n".join(parts) if parts else f"Conversation with {len(rows)} turns"
...
def build_resume_context_text(
    *,
    summary: AIConversationSummary | None,
    ...
    artifact_text = summarize_artifacts_for_prompt(
        artifacts[-3:],
        max_chars=min(600, max_chars),
        limit=3,
    )
    if artifact_text:
...
class ContextStore:
    ...
    def get_full_context(
        self, work_id: str
    ) -> tuple[list[AIConversationTurn], AIConversationSummary | None]:
        """
        获取完整上下文（摘要 + 历史）
    ...
    def build_resume_context(
        ...
        *,
        ...
        history, summary = self.get_full_context(work_id)
        return build_resume_context_text(
            summary=summary,
            history=history,
    ...
    def _compress_summary(
        ...
        with conn.cursor() as cur:
            ...
            summary = _build_structured_summary(rows)
