...
PYTHON_SYMBOL_RE = re.compile(r"^(?:class|def)\s+([A-Za-z_][A-Za-z0-9_]*)", re.MULTILINE)
    ...
    for command in commands:
        ...
        for token in command:
            ...
            if stripped.endswith("/") and stripped.startswith("tests/"):
                ...
                if candidate.exists() and candidate.is_dir() and candidate not in scoped_dirs:
...
def _score_test_path_against_query_keywords(
    *,
    ...
    filename_matches = {keyword for keyword in query_keywords if keyword in filename_tokens}
...
def _should_treat_as_expected_red_test_task(*, title: str, body: str) -> bool:
    ...
    if "write failing test" not in normalized_title and "失败测试" not in normalized_body:
        return False
    ...
    return any(marker in body or marker in normalized_body for marker in red_test_markers)
