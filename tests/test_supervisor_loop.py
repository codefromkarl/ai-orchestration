...
def test_run_supervisor_iteration_sync_ready_sql_guards_waiting_sessions(tmp_path):
    ...
    assert (
        "es.status IN ('suspended', 'waiting_internal', 'waiting_external')" in executed
    )
...
def test_run_supervisor_iteration_records_orchestrator_session_id(tmp_path):
    insert_params: list[tuple[object, ...]] = []
    ...
    assert launched == 1
    assert insert_params[0][-1] == "orch-123"


def test_run_supervisor_iteration_skips_launch_when_orchestrator_session_escalates(
    tmp_path,
):
    insert_params: list[tuple[object, ...]] = []
...
def test_run_supervisor_iteration_skips_launch_when_orchestrator_session_verifies(
    tmp_path,
):
    insert_params: list[tuple[object, ...]] = []
    ...
    launched = run_supervisor_iteration(
        ...
        max_parallel_jobs=1,
        launcher=lambda command, log_path: FakeProcess(),
        orchestrator_session_id="orch-verify",
        repository_builder=FakeRepository,
    )
...
def test_run_supervisor_iteration_plan_phase_allows_decomposition_but_blocks_story_workers(
    tmp_path,
):
    insert_params: list[tuple[object, ...]] = []
    ...
    launched = run_supervisor_iteration(
        ...
        max_parallel_jobs=2,
        launcher=lambda command, log_path: FakeProcess(),
        orchestrator_session_id="orch-plan",
        repository_builder=FakeRepository,
    )
